...
/datum/round_event/vent_clog/announce(fake)
	...
	priority_announce("В вентиляционной системе [event_area] обнаружен засор биологического характера.", "Оповещение о загрязнении") //MASSMETA TRANSLATION
...
/datum/round_event/vent_clog/proc/vent_move(datum/source)
	...
	announce_to_ghosts(vent)
	priority_announce("Биологические маркеры указывают на новое место в вентиляционной сети. Новое местоположение: [prob(50) ? "Неизвестно.":"[get_area_name(vent)]."]", "Неопознанные формы жизни") //MASSMETA TRANSLATION
...
/datum/round_event/vent_clog/major/announce(fake)
	var/area/event_area = fake ? pick(GLOB.teleportlocs) : get_area_name(vent)
	priority_announce("В вентиляционной сети обнаружен серьёзный засор биологического характера. Предполагается, что проблема находится в районе [event_area].", "Нападение вредителей")
...
/datum/round_event/vent_clog/critical/announce(fake)
	var/area/event_area = fake ? pick(GLOB.teleportlocs) : get_area_name(vent)
	priority_announce("В вентиляционной сети [event_area] обнаружены потенциально опасные признаки жизнедеятельности.", "Предупреждение о безопасности")
...
/datum/round_event/vent_clog/strange/announce(fake)
	var/area/event_area = fake ? pick(GLOB.teleportlocs) : get_area_name(vent)
	priority_announce("В вентиляционной сети [event_area] зафиксированы аномальные показатели жизнедеятельности.", "Неопознанные формы жизни", ANNOUNCER_ALIENS)
