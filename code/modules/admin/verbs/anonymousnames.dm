...
/datum/anonymous_theme/proc/announce_to_all_players()
	priority_announce("Недавняя административная ошибка в Департаменте Кадровых Ресурсов привела к необходимости полного отзыва всех идентификационных данных и имен до дальнейшего уведомления.", "Потеря индивидуальности экипажа", SSstation.announcer.get_rand_alert_sound())
...
/datum/anonymous_theme/proc/restore_all_players()
	priority_announce("Имена и личности в системе восстановлены.", "Восстановление индивидуальности экипажа", SSstation.announcer.get_rand_alert_sound())
	for(var/mob/living/player in GLOB.player_list)
		if(!player.mind || (!ishuman(player) && !issilicon(player)) || player.mind.assigned_role.faction != FACTION_STATION)
			continue
...
/datum/anonymous_theme/employees/announce_to_all_players()
	priority_announce("В качестве наказания за низкую продуктивность этой станции по сравнению с соседними станциями доступ к именам и личным данным будет ограничен до дальнейшего уведомления.", "Финансовый отчет", SSstation.announcer.get_rand_alert_sound())
...
/datum/anonymous_theme/wizards/announce_to_all_players()
	priority_announce("Ваша станция попала под воздействие меметической угрозы от Федерации Волшебников. Выыы неее выы, и те-перь... Добро пожаловать в Академию, ученики!", "Меметическая угроза", SSstation.announcer.get_rand_alert_sound())
...
/datum/anonymous_theme/spider_clan/announce_to_all_players()
	priority_announce("Ваша станция была продана клану Пауков. Сейчас будут введены новые указания.", "Новое руководство", SSstation.announcer.get_rand_alert_sound())
...
/datum/anonymous_theme/station/announce_to_all_players()
	priority_announce("Подтверждена опасность разрыва реальности 9 уровня около [station_name()]. Всему персоналу надлежит продолжить работу, однако будте бдительны и не спровоцируйте еще больше разрывов.", "Отдел Центрального Командования по трехмерным измерениям", 'sound/announcer/notice/notice1.ogg')
