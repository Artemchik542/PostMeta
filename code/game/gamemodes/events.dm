 ...
 */
/proc/power_failure()
	priority_announce("В энергосети [station_name()] обнаружены перепады напряжения. В целях предосторожности энергосистема станции будет временно обесточена.", "Аварийный отказ сети", ANNOUNCER_POWEROFF) //MASSMETA TRANSLATION
 ...
 */
/proc/power_restore()
	priority_announce("Энергоснабжение станции [station_name()] возобновлено. Приносим извинения за неудобства.", "Питание восстановлено", ANNOUNCER_POWERON) //MASSMETA TRANSLATION
	for(var/obj/machinery/power/apc/C as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/apc))
		if(C.cell && is_station_level(C.z))
...
/proc/power_restore_quick()
	priority_announce("Все накопители энергии типа "SMES" на [station_name()] были заряжены. Приносим вам извинения за неудобства.", "Питание восстановлено", ANNOUNCER_POWERON) //MASSMETA TRANSLATION
	var/list/all_smes = SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/smes)
	for(var/obj/machinery/power/smes/smes as anything in all_smes)
		if(!is_station_level(smes.z))
			continue
